...
#define MOTOR_CAL_PWM_VALUE 205
#define MOTOR_PWM_MAX_VALUE 255
#define MOTOR_PWM_MIN_VALUE 180
#define MOTOR_SEEK_MAX_DURATION 7000
#define MOTOR_STALL_ENCODER_COUNT_LIMIT 5
#define MOTOR_STALL_CHECK_COOLDOWN 25
...
ISR(SPI_STC_vect) {
  if(SPDR == 0) {
    ...
    } else if(lastSpiByteRecieved == 18) {
        //Got request for: set "forward stop" to current encoder value
        spiByteToReturn = 9; //Accepted "set forward stop" command
    } else if(lastSpiByteRecieved == 19) {
        //Got request for: set "backward stop" to current encoder value
        spiByteToReturn = 10; //Accepted "set backward stop" command
    } else if(lastSpiByteRecieved == 20) {
        //Got request for: set "is calibrated" to true
        spiByteToReturn = 137; //Accepted 'set "is calibrated" to true' command
    } else if(lastSpiByteRecieved == 21) {
        //Got request for: set "is calibrated" to false
        spiByteToReturn = 138; //Accepted 'set "is calibrated" to false' command
    } else {
        //Got unknown command
        spiByteToReturn = 2;
    }
...
void loop()
{
  ...
  //if(Serial.available()) {
    ...
    if(gotNewSpiByte && lastSpiByteRecieved > 0) {
        ...
        if(lastSpiByteRecieved == 1) {
            ...
            Serial.println("Got request for: set 'forward stop' to current encoder value");
            encoderCountForwardStop = encoderCount;
            globalState = 0;
        } else if(lastSpiByteRecieved == 19) {
            Serial.println("Got request for: set 'backward stop' to current encoder value");
            encoderCountBackwardStop = encoderCount;
            globalState = 0;
        } else if(lastSpiByteRecieved == 20) {
            Serial.println("Got request for: set 'is calibrated' to true");
            doCalibrationRoutine = false;
            completedCalibrationRoutine = true;
            globalState = 0;
        } else if(lastSpiByteRecieved == 21) {
            Serial.println("Got request for: set 'is calibrated' to false");
            doCalibrationRoutine = false;
            completedCalibrationRoutine = false;
            globalState = 0;
        } else {
            lastSpiByteRecieved = 0;
  ...
  *////////////////
  ...
  //stall check
  if(motorIsOn && millis() >= lastMillisMotorStart + 250 && millis() > lastMillisStallCheck + MOTOR_STALL_CHECK_COOLDOWN) {
    lastMillisStallCheck = millis();
    if( 
        (!motorDirection && lastEncoderCount + MOTOR_STALL_ENCODER_COUNT_LIMIT >= encoderCount) 
        || (motorDirection && lastEncoderCount - MOTOR_STALL_ENCODER_COUNT_LIMIT <= encoderCount)
      ) {
  ...
  if(globalState > 0 && globalState < 3) {
    if(doCalibrationRoutine) {
      ...
      if(startMotorSeek && !motorIsOn && millis() >= lastMillisMotorStart + 1000) {
        ...
        analogWrite(pwmPin, MOTOR_CAL_PWM_VALUE);
  ...
  if(globalState == 3) {
    if(completedCalibrationRoutine) {
      if(millis() <= lastMillisMotorSeekStart + MOTOR_SEEK_MAX_DURATION) { //only seek for x seconds maximum per seek attempt
        ...
        lastMillisMotorStart = lastMillisMotorSeekStart;
      } else {
        //ensure motor is stopped and reset globalState
        analogWrite(pwmPin, 0);
        globalState = 0;
      }
    }
